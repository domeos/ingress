...
`, waitStatus.ExitStatus(), err)
		...
		for {
			conn, err := net.DialTimeout("tcp", "127.0.0.1:"+listenPort, 1*time.Second)
			if err != nil {
				break
			}
			conn.Close()
...
func (n *NGINXController) OnUpdate(ingressCfg ingress.Configuration) ([]byte, error) {
	...
	// round_robin is the default loadBalancing method, 80 is the default listen port
	loadBalanceMethod := os.Getenv("LBMETHOD")
	listenPort := os.Getenv("LISTENPORT")
	if len(listenPort) == 0 {
		listenPort = "80"
	}
