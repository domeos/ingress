...
func checkListenPort(listenPort string) bool {
	if len(listenPort) == 0 {
		return false
	}
	udpaddr, err := net.ResolveUDPAddr("udp", ":"+listenPort)
	if err != nil {
		glog.Errorf("net.ResolveUDPAddr fail: %v", err.Error())
		return false
	}
	...
	if err != nil {
		glog.Errorf("net.ResolveTCPAddr fail: %v", err.Error())
		return false
	}
...
func portIsUsedByNginx(listenPort string) bool {
	if len(listenPort) == 0 {
		return false
	}
	out, err := exec.Command("/bin/sh", "-c", fmt.Sprintf("lsof -i:%v", listenPort)).Output()
	if err != nil {
		glog.Errorf("failed to run lsof -i: %v", err.Error())
		return false
	}
